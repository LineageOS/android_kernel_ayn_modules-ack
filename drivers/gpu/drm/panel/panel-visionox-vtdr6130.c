...
static int retroidpocket_rp6_on(struct visionox_vtdr6130 *ctx)
{
	...
	if (ret < 0) {
		dev_err(ctx->panel.dev, "Failed to transmit PPS: %d\n", ret);
		return ret;
	}

	ret = mipi_dsi_compression_mode(dsi, true);
	if (ret < 0) {
		dev_err(ctx->panel.dev, "Failed to enable compression mode: %d\n", ret);
		return ret;
	}
...
static int visionox_vtdr6130_probe(struct mipi_dsi_device *dsi)
{
	...
	if (ctx->desc->dsc.dsc_version_major || ctx->desc->dsc.dsc_version_minor)
